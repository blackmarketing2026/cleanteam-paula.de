...
$publicUrl = base_url() . '/o.php?token=' . ($sendAsQuote ? $offer['quote_token'] : $offer['token']); $contact = trim($offer['c_salutation'] . ' ' . $offer['c_contact_last_name']); $company = trim((string) $offer['c_name']);
...
$bodyContent = $sendAsQuote
    ? '<p>Guten Tag ' . email_h($contact) . ',</p><p>anbei erhalten Sie Ihren Kostenvoranschlag von CleanTeam für ' . email_h($company) . '.</p><p>Bitte prüfen Sie die Angaben. Über den folgenden Button können Sie den Kostenvoranschlag online ansehen und annehmen. Danach erhalten Sie automatisch Ihre Auftragsbestätigung.</p>' . email_button_html($publicUrl, 'Kostenvoranschlag ansehen und annehmen') . '<p style="color:#51657d;font-size:13px;">Der Link ist bis ' . email_h($validUntil) . ' Uhr gültig.</p>'
...
$message = render_email_template_message($pdo, $bodyContent, ['title' => $sendAsQuote ? 'Ihr Kostenvoranschlag von CleanTeam' : 'Ihr Vertrag von CleanTeam', 'preheader' => $sendAsQuote ? 'Bitte prüfen und annehmen.' : 'Bitte schließen Sie Ihren Vertrag online ab.', 'fromName' => $settings['from_name'] ?? 'CleanTeam', 'signatureText' => $settings['signature'] ?? '', 'signatureContext' => 'offer']);
try {
...
$pdo->prepare($sendAsQuote ? "UPDATE offers SET quote_sent_at = UTC_TIMESTAMP(), quote_status = 'sent' WHERE id = :id" : 'UPDATE offers SET sent_at = UTC_TIMESTAMP() WHERE id = :id')->execute(['id' => $offerId]);
