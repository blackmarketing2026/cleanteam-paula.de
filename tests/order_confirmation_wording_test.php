...
foreach ([$template, $pdf] as $documentSource) {
    ...
    if (str_contains($documentSource, 'Vertrag vom ')) {
        throw new RuntimeException('Der Dokumententitel selbst darf für Bestandskunden nur "Vertrag" heißen, ohne Datum.');
    }
...
foreach ([
    ...
    'Ihre Vertragsaktualisierung',
    'Ihre Vertragsaktualisierung zum Vertrag vom ',
    'Lesen Sie die Vertragsaktualisierung bitte vollständig durch.',
    'Vertrag erfolgreich abgeschlossen',
] as $existingContractWording) {
    if (!str_contains($publicJs, $existingContractWording)) {
        throw new RuntimeException('Bestandskunden sehen im öffentlichen Vertragsprozess nicht durchgängig die Vertragsaktualisierungs-Bezeichnung: ' . $existingContractWording);
    }
}
